...
export function parseMediaLink(url) {
  ...
  if (youTubeId) {
    const thumbnailCandidates = ['maxresdefault', 'sddefault', 'hqdefault', 'mqdefault', 'default'];
    const fallbackThumbnail = thumbnailCandidates
      .map((variant) => `https://i.ytimg.com/vi/${youTubeId}/${variant}.jpg`)
      .find(Boolean);
    return {
      provider: 'youtube',
      id: youTubeId,
      ...
      thumbnails: thumbnailCandidates.map((variant) => `https://i.ytimg.com/vi/${youTubeId}/${variant}.jpg`),
    };
  }
  ...
  if (instagramInfo) {
    ...
    return {
      ...
      thumbnailUrl: `https://ddinstagram.com/${type}/${code}/media/?size=l`,
    };
  }
...
export function getMediaThumbnail(url) {
  ...
  if (Array.isArray(parsed.thumbnails)) {
    const candidate = parsed.thumbnails.find(Boolean);
