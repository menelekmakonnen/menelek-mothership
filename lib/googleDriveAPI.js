...
export function getGoogleDriveImageUrl(fileId) {
  return resolveDriveImage(buildDriveImageVariants(fileId), 'preview');
}

export function getGoogleDriveThumbnailUrl(fileId, size = 512) {
  return `${DRIVE_THUMB_BASE}${fileId}&sz=w${size}`;
}
...
function buildGalleryFromFolder(categoryName, galleryFolder, imageFiles) {
  ...
  const coverVariants = buildDriveImageVariants(imageFiles[0].id);
  ...
  return {
    ...
    items: imageFiles.map((file, index) => {
      const variants = file.imageVariants || buildDriveImageVariants(file.id);
      return {
        id: file.id,
        name: file.name,
