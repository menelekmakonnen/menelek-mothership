...
export const useMediaData = () => {
  ...
  useEffect(() => {
    const loadMediaData = async () => {
      ...
      // Fetch Loremaker data from Google Sheets (GViz - no API key needed!)
      try {
        loremakerData = await fetchLoremakerData();
        console.log('✅ Loremaker (GViz):', loremakerData.items?.length || 0, 'characters');
      } catch (error) {
      ...
      if (!hasAPIKey) {
        ...
        console.warn('========================================');
        console.warn('Photography and AI Albums need an API key');
        console.warn('Create .env.local file with:');
        console.warn('NEXT_PUBLIC_GOOGLE_API_KEY=your_actual_key');
        console.warn('See SETUP_NOW.md for detailed instructions');
        console.warn('========================================\n');
      } else {
