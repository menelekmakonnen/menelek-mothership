...
const InstagramBelt = memo(function InstagramBelt({ label, urls, onOpen }) {
  const beltRef = useRef(null);
  const [seed, setSeed] = useState(0);
  const picks = useMemo(() => randomSample(urls || [], MAX_REELS_PER_BELT), [urls, seed]);
  ...
  useEffect(() => {
    ...
    if (typeof window === "undefined" || !beltRef.current) {
      setShouldLoad(true);
      return undefined;
    }
  ...
  useEffect(() => {
    if (!shouldLoad || !picks.length) return undefined;
    let cancelled = false;
    setMedia([]);
    setState("loading");
    (async () => {
  ...
  return (
    <div ref={beltRef} className="space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setSeed((value) => value + 1)}
            className="px-3 py-1 text-xs border border-white/20 rounded-full text-white/70 hover:text-white hover:bg-white/10 disabled:opacity-40 disabled:cursor-not-allowed"
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 snap-x" aria-label={`${label} reel belt`}>
        {!media.length && state === "loading" ? skeletons : null}
        {media.length
          ? media.map((item) => (
              <button
                key={item.url}
                type="button"
                onClick={() => handleOpen({ label, url: item.url })}
          ...
          : null}
        {state === "error" && !media.length ? (
          <div className="w-full text-sm text-white/60">Unable to load Instagram previews right now.</div>
        ) : null}
      </div>
    </div>
  );
...
const MMMGalleries = memo(function MMMGalleries() {
  ...
  const handleOpen = useCallback((payload) => setModal(payload), []);
  ...
  return (
    <section id="galleries" className="py-16">
      <div className="max-w-7xl mx-auto px-6 space-y-10">
        ...
        {belts.map(([label, urls]) => (
          <InstagramBelt key={label} label={label} urls={urls} onOpen={handleOpen} />
        ))}
      </div>
