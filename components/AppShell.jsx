...
const TRUSTED_BY = ["Netflix Pitch Labs", "Left Hook Gym", "Nubian Skin", "ICUNI", "Paramount (UK)"];

const TESTIMONIALS = [
  {
    quote: "Menelek translated a loose mood-board into a cinematic short that landed us new clients within weeks.",
    author: "Amelia Cole",
    role: "Creative Director, Nubian Skin",
  },
  {
    quote: "From pre-production discipline to fast post turnarounds, he’s the safest pair of hands I know.",
    author: "Daniel Green",
    role: "Producer, ICUNI Studios",
  },
  {
    quote: "The boxing doc captured heart and grit—we sold out our pilot screening off that edit alone.",
    author: "Coach Marlon",
    role: "Founder, Left Hook Gym",
  },
];
...
const SITE_META = {
  title: "Menelek Makonnen — Director & Worldbuilder",
  description:
    "Filmmaker and creative director crafting cinematic worlds, branded films, and AI-driven visuals for clients who want story-first impact.",
  siteUrl: "https://menelekmakonnen.com",
  image: "https://menelekmakonnen.com/og-image.jpg",
};
  ...
  "@graph": [
    {
      ...
      jobTitle: "Director & Creative Technologist",
      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-10 items-center">
        <div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Button href={LINKS.loremakerSite} icon={ExternalLink} target="_blank" rel="noreferrer">
              Explore Loremaker
            </Button>
            <Button variant="ghost" onClick={() => goTo("value-calculator")}>
              Get a Free Quote
            </Button>
          </div>
        </div>
        ...
        >
          ...
          <div className="mt-3 aspect-video w-full rounded-2xl overflow-hidden border border-white/10 relative group select-none">
...
function SocialProof() {
  ...
  return (
    <section className="pb-10" aria-label="Client trust signals">
      <div className="max-w-7xl mx-auto px-6 space-y-6">
        <div className="rounded-3xl border border-white/15 bg-white/5 backdrop-blur-xl px-4 py-3 flex flex-wrap items-center justify-center gap-3 text-xs sm:text-sm text-white/70">
          <span className="uppercase tracking-[0.3em] text-white/55">Trusted by</span>
          {TRUSTED_BY.map((name) => (
            <span key={name} className="px-3 py-1 rounded-full border border-white/20 bg-white/10 text-white/80">
              {name}
            </span>
          ))}
        </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <footer className="mt-4 text-sm text-white/65">
                  <div className="font-medium text-white/85">{testimonial.author}</div>
                  <div>{testimonial.role}</div>
                </footer>
              </motion.blockquote>
            </AnimatePresence>
            <div className="absolute bottom-5 right-5 text-xs text-white/50">{index + 1}/{TESTIMONIALS.length}</div>
          </Card>
          <Card className="flex flex-col justify-center gap-5">
            {HERO_METRICS.map((metric) => (
              <div key={metric.label} className="flex items-baseline justify-between border-b border-white/10 pb-2 last:border-b-0 last:pb-0">
    ...
    <div className="mt-6">
      ...
      <div className="rounded-2xl border border-white/15 bg-gradient-to-br from-white/10 to-white/5 p-4 shadow-[0_8px_40px_rgba(0,0,0,0.35)]">
            ...
            return (
              <div key={p.key} className="flex items-center gap-3">
                ...
                <div className="relative h-10 flex-1">
                  ...
                  >
                    <div
                      className={cn("absolute inset-0", isTouch ? "cursor-default" : "cursor-grab active:cursor-grabbing")}
                      onPointerDown={isTouch ? undefined : (e) => onPointerDown(e, idx, "move")}
                      title={isTouch ? "Timeline editing is simplified on touch" : "Drag to move"}
                    />
                    <div
                      className={cn("absolute left-0 top-0 h-full w-3", isTouch && "pointer-events-none")}
                      onPointerDown={isTouch ? undefined : (e) => onPointerDown(e, idx, "left")}
                      title={isTouch ? "" : "Drag to adjust start"}
                    >
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[2px] bg-white/80" />
                    </div>
                    <div
                      className={cn("absolute right-0 top-0 h-full w-3", isTouch && "pointer-events-none")}
                      onPointerDown={isTouch ? undefined : (e) => onPointerDown(e, idx, "right")}
                      title={isTouch ? "" : "Drag to adjust end"}
                    >
                      <div className="absolute right-0 top-1/2 -translate-y-1/2 h-5 w-[2px] bg-white/80" />
                    </div>
        ...
        <div className="mt-2 text-white/65 text-xs">
          Default is <b>Finish‑to‑Start</b>. {isTouch ? "On touch devices, tweak the sliders above and I’ll keep these blocks in sync." : "Drag edges to overlap phases."} Bars snap by <b>day</b>. Days mode auto‑enables when total ≤ 2 weeks.
        </div>
      </div>
    </div>
  );
...
function ChatbotWidget() {
  ...
  return (
    <div className="w-[260px] sm:w-[320px] bg-black/70 border border-white/15 rounded-3xl backdrop-blur-xl shadow-[0_18px_60px_rgba(0,0,0,0.55)] overflow-hidden">
      <div
...
function MMMBelt({ collection, onOpen }) {
  ...
  const beltItems = isTouch ? media : duplicated;
  ...
  return (
    <div className="rounded-3xl border border-white/15 bg-white/5 backdrop-blur-xl p-6">
      ...
      <div className={cn("relative", isTouch ? "overflow-x-auto" : "overflow-hidden")} style={{ touchAction: isTouch ? "pan-x" : "auto" }}>
        {beltItems.length ? (
          <motion.div
            className="flex gap-4"
            animate={isTouch ? undefined : { x: ["0%", "-50%"] }}
            transition={isTouch ? undefined : { duration: 48, repeat: Infinity, ease: "linear" }}
          >
            {beltItems.map((item, idx) => {
              const baseIndex = media.length ? idx % media.length : idx;
              const isActive = hovered === baseIndex;
              const isDimmed = hovered !== null && !isActive;
              return (
                <button
                  key={`${collection.id}-${idx}`}
                  onMouseEnter={isTouch ? undefined : () => setHovered(baseIndex)}
                  ...
                  className={cn(
                    ...
                    isTouch ? "scale-100" : isActive ? "scale-110 z-20" : isDimmed ? "scale-90 opacity-60" : "scale-100"
                  )}
                  title="Open lightbox"
                >
                  <img
...
function FloatingButtons({ onOpenContact }) {
  return (
    <div className="fixed bottom-4 right-4 z-40 flex flex-col items-end gap-3 sm:gap-4">
      <ChatbotWidget />
      <div className="flex flex-col items-end gap-2 sm:flex-col">
        <button
...
function SectionNav({ variant = "floating", className = "" }) {
  ...
  if (selectIdRef.current === null) {
    selectIdRef.current = `section-nav-${variant}-${Math.random().toString(36).slice(2, 7)}`;
  }
  ...
  if (isMobile) {
    return (
      <div className={cn("w-full", className)}>
        <label htmlFor={selectIdRef.current} className="sr-only">
          Jump to section
        </label>
        <select
          id={selectIdRef.current}
          className="w-full px-4 py-3 rounded-2xl border border-white/20 bg-white/10 text-white/90"
          defaultValue=""
          onChange={(e) => {
        ...
        >
          ...
          {SECTION_LINKS.map((link) => (
            <option key={link.id} value={link.id} className="text-black">
              {link.label}
            </option>
          ))}
        </select>
...
export default function AppShell() {
  ...
  const jsonLd = useMemo(() => JSON.stringify(JSON_LD), []);
